...
{
    public class EventBus(IMediator mediator, IOutboxListener outboxListener, IEventListener eventListener) : IEventBus
    {
        private readonly IMediator _mediator = mediator ?? throw new Exception($"Missing dependency '{nameof(IMediator)}'");
        private readonly IOutboxListener _outboxListener = outboxListener;
